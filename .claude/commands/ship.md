...
→ apps/app-003/ を llm-cost.southernlabs.com にデプロイ
```
...
```bash
vercel domains add {サブドメイン名}.southernlabs.com
```
...
const apps: { name: string; description: string; url: string }[] = [
  // 既存のエントリ...
  { name: "{アプリ名}", description: "{一言説明}", url: "https://{サブドメイン名}.southernlabs.com" },
];
```
...
```
| {番号} | {今日の日付} | {アプリ名} | https://{サブドメイン名}.southernlabs.com | |
```
...
curl -H "Content-Type: application/json" \
  -d '{"content": "🚀 新アプリ公開: **{アプリ名}**\n{一言説明}\nhttps://{サブドメイン名}.southernlabs.com"}' \
  "$DISCORD_WEBHOOK_URL"
```
...
```
{アプリ名}：公開 ({サブドメイン名}.southernlabs.com)
```
...
| 本番URL疎通 | `curl -s -o /dev/null -w "%{http_code}" https://{サブドメイン名}.southernlabs.com` が 200 |
| OGP | `curl -s https://{サブドメイン名}.southernlabs.com/opengraph-image` が画像を返す |
